    ...
    val task       = new Runnable:
      def run() =
        try
          ...
          case e =>
            // Increment the number of exceptions, and finish this task
      ...
      def run() =
        while (true) {
          ...
          try
            ...
            val nextTasks = getNextTasks(alg) // Get the next tasks specified by the algorithm and its controller
